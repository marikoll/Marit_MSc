...
This chapter describes the setup and altered modules of the Oslo CTM3 (for general information concerning the CTM3, see Chapter \ref{chapt:OsloCTM3}). The altered modules that are connected to the theory outlined in Chapter \ref{Chap:CTM3theory_ocean_hetReact} is presented here, whereas many of the technical aspects of setting up the Oslo CTM3 can be found in the appendix. 
...
The loss in $\chem{O_3}$ was added assuming a low-$\chem{NO_x}$ regime explained in Section \ref{sec:O3-NO}. The reactions leading to ozone production or loss was therefore implemented using the low-$\chem{NO_x}$ regime when integrated.
