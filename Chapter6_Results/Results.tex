...
\input{Chapter6_Results/images/polarHBr_noCl}

\input{Chapter6_Results/images/polarBrO_noCl}
...
\input{Chapter6_Results/images/vertHBr_newRestart}

\input{Chapter6_Results/images/polarHBr_newRestart}

\input{Chapter6_Results/images/polarHOBR_newRestart}
...
\clearpage
\input{Chapter6_Results/images/polarBrO_newRestart}
...
\input{Chapter6_Results/images/vertHBr_HTWO_step3}

%\input{Chapter6_Results/images/polarHBr_HFOUR_step3}
\input{Chapter6_Results/images/polarHBr_HTWO_step3}

\input{Chapter6_Results/images/polarHOBr_HTWO_step3}
...
\input{Chapter6_Results/images/polarBrO_HTWO_step3}

\clearpage
\subsubsection{Changing Henry's law coefficient and photodissociation of \chem{HOBr}}
...
This branch was initialized with the restart file provided from \cite{StefaniePersonal} (see Section \ref{subsec:restart_files}) and ran for 6 months runs at \texttt{HTWO} resolution. The resulting ozone mixing ratio at the four stations can be seen in Figure \ref{fig:ozone_2001_step4}. The ozone concentration in the final version starts out lower than what was measured at the different stations, and lower than the other model runs. At the end of the period, the concentration in the final version increases. %Naturlig at det tar litt tid for at det skal stabilisere seg, fordi restartfila har helt forskjellig kjemi --> burde hatt en spin up både for HTWO og ny kjemi


\input{Chapter6_Results/images/ozone_2001_step4}
\medskip

Figure \ref{fig:vertHBr_step4} contains the vertical profile of \chem{HBr} above the fours stations up to 600 hPa. The concentration is on the order of $10^{-15}$ mol/mol ($10^{-3}$ ppt). The polar concentration in the first model layer is shown in Figure \ref{fig:polarHBr_step4}. The concentration is on the order of $10^{-7}$ gm$^{-3}$. 
...
The corresponding \chem{HOBr}-concentration can be seen in Figure \ref{fig:polarHOBr_step4}. Seen in relation with Figure \ref{fig:polarHBr_step4}, there is still an anti-correlation present between the \chem{HBr} and \chem{HOBr}.

\input{Chapter6_Results/images/polarO3_step4}

\medskip

In Figure \ref{fig:polarO3_step4}, it is evident that the low $\chem{O_3}$-concentration higher \chem{HBpassing over Barrow r}-concentration to the right of Barrow 
