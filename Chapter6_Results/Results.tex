...
Figure \ref{fig:vertHBr_noCl} shows the vertical column above the Alert, Barrow, Summit and Zeppelin of the mixing ratio of \chem{HBr}. The mixing ratio is on the order $10^{-15} \sim 0.001$ ppt. The vertical distribution appears to be constrained with higher concentration in the lower layers at Alert, whilst increasing with altitude at Zeppelin. Seen in relation with Figure \ref{fig:polarHBr_noCl}, the concentration in the lowest layer across the Arctic is on the order of $10^{-10} - 10^{-11} g m^{-3}$
...
\subsubsection{Initializing Branch \ref{def:BE_PD_noCl} With a Higher \chem{HBr} Concentration}\label{sec:step2_result}
...
\subsubsection{Hard-Coding Photodissociation and Adjusting the Henry'Law Coefficient}\label{sec:res_HDP_H}
...
\ref{fig:vertHBr_HFOUR_step3} and \ref{fig:vertHBr_HTWO_step3} contains the vertical column of the \chem{HBr} mixing ratio at \texttt{HFOUR} and \texttt{HTWO} resolution, respectively. The mixing ratio remains around $3\times10^{-10}$ mol mol$^{-1} \sim 300 ppt$ for both the tests. Likewise, Figures \ref{fig:polarHBr_HFOUR_step3}-\ref{fig:polarHBr_HTWO_step3} contains concentrations of \chem{HBr} on the order of $6\times10^{-6} $g m$^{-3}$
